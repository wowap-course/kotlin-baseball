...
class RefereeTest () {
    ...
    private val opponent = Opponent(referee)
    @Test
    fun `상대방의 숫자를 알고 있다`() {
    ...
    fun `숫자에 대해 낫싱을 판단할 수 있다`() {
        val answer = "456"
        assertThat(referee.refereeNumber(opponent, answer)).isEqualTo("낫싱")
    }
    ...
    fun `입력한 수에 대해 '볼'을 판단할 수 있다`(answer : String) {
        ...
        assertThat(referee.refereeNumber(opponent, answer)).isEqualTo("${ball}볼")
    }
    ...
    fun `입력한 수에 대해 '스트라이크'를 판단할 수 있다`(answer : String) {
        ...
        assertThat(referee.refereeNumber(opponent, answer)).isEqualTo("${strike}스트라이크")
    }
}
